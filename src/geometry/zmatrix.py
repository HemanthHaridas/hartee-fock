...
class ZMatrix(base_geometry.BaseGeometry):
    ...
    Attributes:
        ...
        coords_internal (numpy.ndarray): Flattened 1D representation of coords.
        natoms (int): Number of atoms in the system.
        charge (int): Total molecular charge.
        multi (int): Spin multiplicity of the system.
        atomicnumbers (numpy.ndarray): Array of atomic numbers.
        atomicmasses (numpy.ndarray): Array of atomic masses.
    ...
    def _from_zmatrix_string(self, zmatrix_string: str) -> None:
        ...
        atomic_numbers = [tables.ATOMIC_NUMBER[symbol] for symbol in atoms]
        self.atomicnumbers = numpy.array(atomic_numbers, dtype=int)

    def _place_third_atom(
        self,
        bonded_to: numpy.ndarray,
        angle_from: numpy.ndarray,
        distance: float,
        angle_deg: float,
    ) -> numpy.ndarray:
        """
        ...
        """
        # Compute bond vector
        bond_vector = numpy.array(bonded_to) - numpy.array(angle_from)
        bond_norm = numpy.linalg.norm(bond_vector)
        ...
        perp = numpy.array([bond_vector[1], -bond_vector[0], 0.0])
        perp_norm = numpy.linalg.norm(perp)
        ...
        rotated = (
            numpy.cos(angle_rad) * bond_vector
            + numpy.sin(angle_rad) * perp
        )

        # Place new atom
        return numpy.array(bonded_to) + distance * rotated

    def _place_atom_with_dihedral(
        self,
        bonded_to: numpy.ndarray,
        angle_from: numpy.ndarray,
        dihedral_from: numpy.ndarray,
        distance: float,
        angle_deg: float,
        dihedral_deg: float,
    ) -> numpy.ndarray:
        ...
        p_bond = numpy.array(bonded_to)
        p_angle = numpy.array(angle_from)
        p_dihedral = numpy.array(dihedral_from)
        ...
        if n_norm < 1e-10:
            ...
            perp = numpy.array([1.0, 0.0, 0.0]) if abs(v1[0]) < 0.9 else numpy.array([0.0, 1.0, 0.0])
            n = numpy.cross(v1, perp)
        ...
        phi = numpy.radians(dihedral_deg)
